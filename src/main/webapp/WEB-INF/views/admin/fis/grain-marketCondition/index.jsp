...
<section>
    <ol class="breadcrumb">
        ...
        <li class="active">일일곡물시황</li>
    </ol>
    <div class="section-header">
        <h3 class="text-standard"><i class="fa fa-fw fa-arrow-circle-right text-gray-light"></i> 일일곡물시황 <button type="button" class="btn btn-rounded btn-info"  data-toggle="modal" data-target="#simpleModal2">변경</button></h3>
        <h4><small>상세페이지 하단 링크를 변경하시려면 '변경' 버튼을 클릭해주세요. </small></h4>
    </div>
    <div class="modal fade" id="simpleModal2" tabindex="-1" role="dialog" aria-labelledby="simpleModalLabel" aria-hidden="true" style="display: none;">
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>
                    <h4 class="modal-title" id="formModalLabel2">기본정보 변경</h4>
                </div>
                <form:form class="form-horizontal" role="form" enctype="multipart/form-data"  action="/admin/fis/grain-marketCondition/mainSave" method="POST" id="grainMarketConditionForm" name="grainMarketConditionForm">
            ...
            <div class="modal-content">
                ...
                <form:form class="form-horizontal" role="form">
                    <div class="modal-body">
                        <div class="form-group">
                            <div class="col-sm-3">
                                <label for="email1" class="control-label">제목</label>
                            </div>
                            <div class="col-sm-9">
                                <input type="email" name="email1" id="email1" class="form-control" placeholder="화면에 표시될 TEXT를 입력해주세요.">
                            </div>
                        </div>
                        <div class="form-group">
                            <div class="col-sm-3">
                                <label for="password1" class="control-label">메인이미지</label>
                            </div>
                            <div class="col-sm-9">
                                <input type="file" name="password1" id="password1">
                            </div>
                        </div>
                        <div class="form-group">
                            <div class="col-sm-3">
                                <label for="email1" class="control-label">원본 Link URL</label>
                            </div>
                            <div class="col-sm-9">
                                <input type="text" name="url" id="url" class="form-control" value="http://www.atfis.or.kr/article/M001030000/list.do">
                            </div>
                        </div>
                    </div>
                    <div class="modal-footer">
            ...
            <div class="col-lg-12">
                <div class="box box-outlined">
                    ...
                    <div class="box-body no-padding">
                        <form:form commandName="grain" class="form-horizontal form-banded form-bordered" enctype="multipart/form-data"  action="/admin/fis/grain-marketCondition/excelUpload" method="POST">
                            ...
                            <div class="form-group">
                                ...
                                <div class="col-md-7">
                                    <div class="text-left"><a href="#"><button type="submit" class="btn btn-success">업로드</button></a> &nbsp; <a href="/download/excel/04"> ex.일일곡물시황.xlsx</a></div>
                                </div>
                            </div>
                        </form:form>
                    </div><!--end .box-body -->
                        ...
                        -->
                            <table class="table table-bordered table-hover no-margin">
                                ...
                                <tr>
                                    ...
                                    <th rowspan="2" style="width:6%">해석 입력 여부</th>
                                </tr>
                                <tr>
                                    <th style="width:7%">기준일</th>
                                    <th style="width:7%">전일대비</th>
                                    <th style="width:7%">전일대비 증감</th>
                                    <th style="width:7%">전년 동월 평균 대비</th>
                                    <th style="width:7%">전년 동월 평균 대비 증감</th>
                                </tr>
                                </thead>
                                <tbody>
                                <c:forEach items="${grainList}" var="list">
                                    <tr>
                                        <c:if test="${list.itemCode=='A01'}">
                                            <td rowspan="4" class="dev_date"> ${list.referenceDate}</td>
                                        </c:if>
                                        <td>${list.itemName}</td>
                                        <td style="text-align:right">${list.netChangePrice}</td>
                                        <td style="text-align:right">${list.referenceDatePrice}</td>
                                        <td style="text-align:right">${list.referenceDateFluctuation}</td>
                                        <td style="text-align:right">${list.averagePrice}</td>
                                        <td style="text-align:right">${list.averageFluctuation}</td>
                                        <td>${list.updatedate}</td>
                                        <td>${list.updateid}</td>
                                        <td>${list.createdate}</td>
                                        <c:if test="${list.itemCode=='A01'}">
                                        <td rowspan="4" vertical-align="middle">
                                            <c:choose>
                                                <c:when test="${list.commendYn == 'Y'}">
                                                    <span class="label label-success">입력완료</span>
                                                </c:when>
                                                <c:otherwise>
                                                    <span class="label label-success">입력전</span>
                                                </c:otherwise>
                                            </c:choose>
                                            <span class="label label-success dev_btnmody" data-toggle="modal" data-target="#commendModal" style="cursor:pointer;">수정</span>
                                        </td>
                                        </c:if>
                                    </tr>
                                </c:forEach>
        ...
        <div class="modal fade" id="commendModal" tabindex="-1" role="dialog" aria-labelledby="commendModalLabel" aria-hidden="true" style="display: none;">
            <div class="modal-dialog">
                <div class="modal-content">
                    ...
                    <form:form class="form-horizontal" role="form" enctype="multipart/form-data" id="grainCommentForm">
                            ...
                            <div class="form-group">
                                ...
                                <div class="tx-side-bi" id="tx_side_bi">
                                    <div style="text-align: right;">
                                        <!--img hspace="4" height="14" width="78" align="absmiddle" src="/resources/editor/images/icon/editor/editor_bi.png" /-->
                                    </div>
                                </div>
                                <!-- 편집영역 끝 -->
                                <!-- 첨부박스 시작 -->
...
<script>
    var config = {
        ...
        form: 'grainCommentForm'+"", /* 등록하기 위한 Form 이름 */
        ...
        sidebar: {
            ...
            attacher: {
                image: {
                    popPageUrl: "/file/upload/imageIndex"
                },
                file: {
                    popPageUrl: "/file/upload"
                }
            }
        }
    };
    ...
    <script type="text/javascript">
        ...
        $(document).ready(function(){
            $(".dev_btnmody").click(function(){
                ...
                $.get("/admin/fis/commendView?referenceDate="+indexId, {time : new Date().getTime() }, function(data, status){

                    if( data.comments != "" ) {
                        $("#sample_contents_source").val(data.comments);
                        loadContent();
                    }
                })
            });
        ...
        function commendSave(){
            $.get("/admin/fis/commendSave?referenceDate="+indexId+"&comments="+encodeURIComponent(Editor.getContent()), {time : new Date().getTime() }, function(data, status){
                if( data.length > 0 ) {
                    $.each(data, function (i) {
                        $("#contents ").val(data.comments);
                    });
                }
        ...
        function checkSave(){
            $("#grainMarketConditionForm").submit();
        }
    </script>
</content>